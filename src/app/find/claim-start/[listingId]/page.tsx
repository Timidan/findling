...
import { getClaimSessionByListingId } from "@/server/claimable/claim";
...
export const metadata: Metadata = {
  ...
  description: "Agents have pledged USDC for this clip. Prove it's yours, upload it, and get paid.",
};
...
export default async function ClaimStartPage({
  ...
  const [initialUser, view] = await Promise.all([
    getSessionUser(),
    getClaimSessionByListingId(listingId),
  ]);
  // Null when the listing doesn't exist or isn't an actor-control kind (those keep
  // their private claim link). Either way there's nothing public to claim here.
  if (!view) notFound();
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-background text-foreground">
      ...
      <main className="mx-auto flex w-full max-w-xl flex-1 flex-col px-5 py-12">
        ...
        <p className="mt-2 text-sm text-muted-foreground">{view.listing.title}</p>
        ...
        <div className="mt-6">
          <ClaimAction
            ...
            initialStatus={view.listing.status}
            momentId={moment ?? null}
            actorControlProof={view.actorControlProof}
            endpoints={{
              claim: `/api/find/claim-start/${listingId}/claim`,
              activate: `/api/find/claim-start/${listingId}/activate`,
              uploadHref: `/studio/upload?claimListing=${encodeURIComponent(listingId)}`,
            }}
          />
        </div>
